...
应用默认不渲染主窗口，可通过托盘图标打开主界面。

## 主窗口懒加载与自动销毁

- 应用启动时默认不创建主窗口 WebView（仅托盘与后台功能常驻）。
- 在托盘点击“打开主窗口”或托盘左键点击后，才会创建并显示主界面。
- 主界面关闭后会先隐藏，并在 3 分钟后自动销毁主窗口实例。
- 若在 3 分钟内再次从托盘打开主界面，会取消销毁并复用/重建窗口。
