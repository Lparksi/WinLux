...
- [要件](#要件)
- [クイックスタート](#クイックスタート)
- [メインウィンドウの遅延生成と自動破棄](#メインウィンドウの遅延生成と自動破棄)
- [スクリプト](#スクリプト)
- [検証](#検証)
- [プロジェクト構成](#プロジェクト構成)
...
アプリは起動時にメインウィンドウを描画せず、トレイアイコンから操作できます。

## メインウィンドウの遅延生成と自動破棄

- 起動時はメイン WebView ウィンドウを作成せず、トレイとバックグラウンド機能のみ常駐します。
- トレイ左クリックまたは「メインウィンドウを開く」で、初めて UI を生成して表示します。
- メインウィンドウを閉じると一度非表示になり、3 分後にウィンドウインスタンスを自動破棄します。
- 3 分以内にトレイから再度開いた場合は、破棄待機をキャンセルしてウィンドウを維持/再生成します。
